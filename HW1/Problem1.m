...
plot(w, phi);
title("Phase delay of H(z) with \alpha:-.8")
xlim([0,pi]);
...
plot(w, gd);
title("Group delay of H(z) with \alpha:-.8")
xlim([0,pi]);
ylabel("Phase Delay (samples)");
xlabel("Frequency (rad)")
...
% Plot Results
surf(T,F,(abs(S)),'edgecolor','none'); axis tight;view(0,90);
title('Impulse response of Butterworth filter under WGN');
ylabel('Frequency (Hz)');
xlabel('Time (s)');
set(gca,'YScale','log')
